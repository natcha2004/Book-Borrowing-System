function populateHistoryTable() {
    ...
    fetch(`http://localhost:8080/api/myNowBorrow/${username}`)
        .then(response => {
            ...
            data.forEach(book => {
                const row = document.createElement('tr');
                                                        ...
                                                        ['bid', 'bname', 'borrow_date', 'return_date'].forEach(property => {
                                                            ...
                                                            if (property === 'borrow_date') {
                                                                if (book[property]) {
                                                                    const date = new Date(book[property]);
                                                                    cell.textContent = date.toLocaleDateString();
                                                                } else {
                                                                    cell.textContent = 'Please pick up the book at the counter in 3 days.'; // Display message if borrow_date is null
                                                                }
                                                            } else if (property === 'return_date') {
                                                                if (book[property]) {
                                                                    const date = new Date(book[property]);
                                                                    cell.textContent = date.toLocaleDateString();
                                                                } else {
                                                                    cell.textContent = ''; // Display null if return_date is null
                                                                }
                                                            } else {
